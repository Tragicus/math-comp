...
Fixpoint eqseq (T : eqType) (s1 s2 : seq T) {struct s2} :=
  match s1, s2 with
  | [::], [::] => true
...
Lemma eqseqP T : Equality.axiom (@eqseq T).
Proof.
...
HB.instance Definition _ (T : eqType) := hasDecEq.Build (seq T) (@eqseqP T).
...
#[hnf] HB.instance Definition _ := Equality.copy bitseq (seq bool).
...
apply: (iffP idP) => [eq_txs | [i [u [Dt eq_us]]]].
  have /(@rot_to _ _ _)[i u Dt]: x \in t by rewrite (perm_mem eq_txs) mem_head.
...
case/forall_cons => /(@rot_to _ _ _)[i s3 def_s2] ss12 le_s21.
...
have /(@rot_to _ _ _)[i s' def_s1]: x \in s1 by rewrite (perm_mem eq_s12) mem_head.
...
by case: ifP => [/(@mem_mask _ _) //|_]; apply: IH.
Qed.
...
exists ((x \in s1) :: m); have [|/(@rem_id _ _)<-//] := boolP (x \in s1).
by move/(@perm_to_rem _ _)/permPl->; rewrite perm_cons.
Qed.
...
Proof.
apply: (iff_trans (count_maskP s1 s2)); split=> [[m _]|[_/subseqP[m sm ->]]]; last by exists m.
...
Proof.
elim: s => //= x s IHs; apply: (iff_trans forall_cons); split=> [[->] /IHs -> //|fgE].
by inversion fgE; split=> //; apply/IHs.
Qed.
...
have /(@rot_to _ _ _)[k s1 Ds]: x \in s by rewrite (perm_mem Est) mem_head.
